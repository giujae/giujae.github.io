...
요구 페이징은 처음부터 모든 데이터를 메모리에 적재하지 않고, CPU가 요청할 때 프로세스를 메모리에 올리는 것을 의미한다.
...
어제 공부했던 메모리 할당 정책 중 등장했던 개념으로 가용 블록을 탐색할 때 사용하는 리스트 였다. 어제는 묵시적 리스트(Implicit free list)로 가정하고 설명했지만, 오늘은 좀 더 알아보도록 했다.
